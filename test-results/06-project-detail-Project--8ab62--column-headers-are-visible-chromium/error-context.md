...
- Name: 06-project-detail.spec.ts >> Project Detail page >> task list column headers are visible
- Location: tests\e2e\06-project-detail.spec.ts:54:7
...
Locator: getByText(/assigned/i)
Expected: visible
Error: strict mode violation: getByText(/assigned/i) resolved to 2 elements:
    1) <div class="hidden md:block">Assigned</div> aka getByText('Assigned', { exact: true })
    2) <span class="text-xs text-muted-foreground">Unassigned</span> aka getByText('Unassigned')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText(/assigned/i)
