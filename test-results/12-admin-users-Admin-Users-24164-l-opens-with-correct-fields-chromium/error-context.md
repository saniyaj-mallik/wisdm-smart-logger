...
- Name: 12-admin-users.spec.ts >> Admin Users page >> Edit User modal opens with correct fields
- Location: tests\e2e\12-admin-users.spec.ts:48:7
...
Locator: getByRole('dialog').getByText(/active/i)
Expected: visible
Error: strict mode violation: getByRole('dialog').getByText(/active/i) resolved to 2 elements:
    1) <span>Active</span> aka getByRole('combobox').filter({ hasText: 'Active' })
    2) <option selected value="active">Active</option> aka locator('select').nth(1)

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByRole('dialog').getByText(/active/i)
